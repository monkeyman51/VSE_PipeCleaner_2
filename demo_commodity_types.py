    ...
    with open(csv_file_path) as file:
        ...
        for row in file_data:
            ...
            if ticket_number.isdigit():
                state = str(row[4]).upper()

                if state == "SIGNED OFF" or state == "CANCELED":
                    pass
                else:
...
def get_commodity_types(tickets_data: dict) -> list:
    ...
    for ticket_number in tickets_data:
        ...
        if str(ticket_number).isdigit():
            ...
            table_data: dict = ticket_data.get("table_data")

            if table_data:
                request_type: str = table_data.get("request_type")
...
def main() -> None:
    ...
    """
    csv_file_path: str = "settings/all_ticket_numbers.csv"
