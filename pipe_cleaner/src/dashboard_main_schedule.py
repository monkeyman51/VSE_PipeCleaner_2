...
def set_issue_structure(worksheet, structure, sheet_title, site_location, total_issues,
    ...
    worksheet.merge_range('E6:F6', f'Still Underdevelopment', structure.red_middle_18)
...
def process_ado_data(azure_devops_data: dict) -> dict:
    """
    Gets data gathered from all unique TRRs associated within VSE from the Console Server.

    Structures the ADO data into meaningful information for excel output to display and list engineers workload.

    :param azure_devops_data: ADO data gathered from Console Server
    :return:
    """
    import json
    # print(json.dumps(azure_devops_data, sort_keys=True, indent=4))
    # with open('pipe_cleaner/src/process_data/console_server_data_sample.py', 'w') as f:
    #     f.write(str(azure_devops_data))
    # input()
    # for item in azure_devops_data:
    #     print(item)
    pass
...
def main_method(ado_data: dict, console_server_data: dict, workbook, structure, site_location: str, all_issues,
    ...
    process_ado_data(console_server_data)
    ...
    create_breakdown_graph(console_server_data, workbook, worksheet_issues, 'Issues', mismatch_tally, missing_tally)
