...
def add_all_serial_data(console_server_data: dict, current_setup: dict):
    ...
    for index, serial_data in enumerate(all_serial_data, start=0):
        ...
        if 'ALIVE' in status.upper():
            worksheet.write(f'D{index + current_position}', 'ACTIVE', current_color)

        elif 'DEAD' in status.upper():
            worksheet.write(f'D{index + current_position}', 'INACTIVE', structure.light_red_middle_11)
...
def add_header_sheet_title(current_setup: dict) -> None:
    ...
    worksheet.write('B5', f'  Console Server - Commodity Serial Number', structure.blue_font_22)
...
def create_personal_issues_sheet(excel_setup: dict) -> dict:
    ...
    excel_setup['sheet_title']: str = 'All Serials'
