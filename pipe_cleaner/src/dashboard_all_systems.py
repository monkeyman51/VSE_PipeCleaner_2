...
def write_pipe_name_column(letter: str, min_number: int, worksheet, structure,
                           console_server_data: dict, vetted_console_server: dict):
    """
    Writes Pipe Name column in excel output
    :param vetted_console_server:
    ...
    for index, pipe_name in enumerate(vetted_console_server):
        machine_names: dict = console_server_data.get(pipe_name, {}).get('pipe_data', 'None')
        processed_pipe_name: str = process_pipe_name(pipe_name)
        pipe_order: int = 0
        end_pipe_section: int = next_min_number
        temp_end = end_pipe_section - 2

        number_of_machines: int = len(machine_names)
        ...
        worksheet.merge_range(f'{letter}{next_min_number}:{letter}{max_number}', processed_pipe_name,
                              structure.blue_middle_huge)

        worksheet.write_url(f'{letter}{min_number}', host_group_url, structure.blue_middle_huge,
                            string=processed_pipe_name)

        if temp_end != 12:
            worksheet.set_row(temp_end, 7.5, structure.white)
        ...
        # for start, machine_name in enumerate(machine_names):
        #
        #     # Fetches information
        ...
        #     number_of_machines: int = len(machine_name)
        #     max_number: int = end_pipe_section + number_of_machines - 1
        #     number_of_cell = str(pipe_order + next_min_number)
        #
    ...
    #         max_number: int = min_number_2 + issues_in_pipe - 1
    #
    #         new_min_number = max_number + 1
    #
    #         worksheet.set_row(max_number, 12, structure.white)
    #
    #         if issues_in_pipe == 1:
    #             worksheet.write_url(f'{letter}{min_number_2}', host_group_url, structure.blue_middle,
    #                                 string=processed_pipe_name)
    #
    #         elif issues_in_pipe >= 2:
    ...
    #                                   processed_pipe_name, structure.blue_middle_huge)
    #
    #             worksheet.write_url(f'{letter}{min_number_2}', host_group_url, structure.blue_middle_huge,
    #                                 string=processed_pipe_name)
    ...
    for index, pipe_name in enumerate(pipe_structure):
        ...
        for start, sku_type in enumerate(sku_type_issues):
            ...
            if sku_issues == 1:
                ...
                # worksheet.write_url(f'{letter}{minimal_number}', '', structure.blue_middle_gigantic,
                #                     string=sku_type)
...
def write_section_column(letter: str, minimal_header_number: int, pipe_structure: dict, processed_issues: dict,
    ...
    #         component_section: str = issue_section.get('section')
    #         color_number = int(number_of_cell) % 2
    #
    #         # Spaces given before text to add readability, makes sure text doesn't rub against border of cell too much
    #         if color_number == 0:
    #             worksheet.write(f'{letter}{number_of_cell}', f'   {component_section}', structure.blue_left)
    #         elif color_number == 1:
    #             worksheet.write(f'{letter}{number_of_cell}', f'   {component_section}', structure.alt_blue_left)
    #
    #         pipe_order_count += 1
    #         issues_count += 1
    #
...
def vet_console_server(console_server_data: dict) -> dict:
    """

    :param console_server_data:
    :return:
    """
    pipes_structure: dict = {}

    real_pipes: list = []
    for potential_pipe in console_server_data:
        if 'Pipe-' in potential_pipe and '[' in potential_pipe and ']' in potential_pipe:
            real_pipes.append(potential_pipe)

    sort_pipes = list(sorted(real_pipes))
    for real_pipe in sort_pipes:
        pipe_data: dict = console_server_data.get(real_pipe, {}).get('pipe_data')

        all_machine_names: list = []
        for machine_name in pipe_data:
            if 'VSE' in machine_name and '-VM-' not in machine_name and 'Z_' not in machine_name \
                    and 'OFFLINE' not in machine_name.upper():
                all_machine_names.append(machine_name)

        sort_machine_names = list(sorted(all_machine_names))
        pipes_structure[real_pipe] = sort_machine_names

    return pipes_structure
...
def add_issue_data(ado_data: dict, console_server_data: dict, all_issues: list, worksheet, structure):
    ...
    vetted_console_server: dict = vet_console_server(console_server_data)
    # sorted_issued: dict = sort_processed_issues(console_server_data)

    write_pipe_name_column('C', minimal_header_number, worksheet, structure, console_server_data, vetted_console_server)
...
def create_breakdown_graph(console_server_data: dict, workbook: xlsxwriter, worksheet: xlsxwriter, sheet_name: str,
    ...
    # Size of Chart
    worksheet.insert_chart('J1', chart_structure, {'x_scale': 2.335, 'y_scale': 0.84})
...
def set_issue_structure(worksheet, structure, sheet_title, site_location, total_issues, total_checks,
    ...
    # worksheet.merge_range('F6:H6', f'Items Under Testing', structure.teal_middle_merge)
...
def set_issue_layout(worksheet, structure):
    ...
    worksheet.set_column('F:F', 18, structure.white)
    worksheet.set_column('G:G', 15, structure.white)
    worksheet.set_column('H:H', 19, structure.white)
    worksheet.set_column('I:I', 19, structure.white)
    worksheet.set_column('J:J', 19, structure.white)
    worksheet.set_column('K:K', 38, structure.white)
    worksheet.set_column('L:L', 38, structure.white)
    worksheet.set_column('M:M', 19, structure.white)
    worksheet.set_column('N:N', 10, structure.white)
    ...
    worksheet.set_column('R:R', 13, structure.white)
...
def main_method(ado_data: dict, console_server_data: dict, workbook, structure, site_location: str, all_issues,
    ...
    """
    # Add to the sheet
    worksheet_all_systems = workbook.add_worksheet('All Systems')

    set_issue_structure(worksheet_all_systems, structure, 'Issues', site_location, len(all_issues), all_checks,
                        pipe_numbers, pipe_cleaner_version)

    add_issue_data(ado_data, console_server_data, all_issues, worksheet_all_systems, structure)

    create_breakdown_graph(console_server_data, workbook, worksheet_all_systems, 'Issues', mismatch_tally, missing_tally)

    # Get rid of all errors showing up in excel cells
    worksheet_all_systems.ignore_errors({'number_stored_as_text': 'A1:XFD1048576'})
